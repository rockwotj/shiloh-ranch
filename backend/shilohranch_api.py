...
class ShilohRanchApi(remote.Service):
    ...
    def needs_update(self, request):
        # The 1 is the numer it is within the request message. Not sure why it is one indexed?
        return updates.get_needs_update(DateTimeField(1).value_from_message(request))

    # Queries
    @Deletion.query_method(path="deletions", http_method="GET", name="deletions", query_fields=("limit", "pageToken", "time_added"))
    def sync_deletions(self, query):
        return query.order(-Deletion.time_added)

    @Category.query_method(path="categories", http_method="GET", name="categories", query_fields=("limit", "pageToken", "time_added"))
    def sync_categories(self, query):
        return query.order(-Category.time_added)

    @Post.query_method(path="posts", http_method="GET", name="posts", query_fields=("limit", "pageToken", "time_added"))
    def sync_posts(self, query):
        return query.order(-Post.time_added)

    @Event.query_method(path="events", http_method="GET", name="events", query_fields=("limit", "pageToken", "time_added"))
    def sync_events(self, query):
        return query.order(-Event.time_added)

    @Sermon.query_method(path="sermons", http_method="GET", name="sermons", query_fields=("limit", "pageToken", "time_added"))
